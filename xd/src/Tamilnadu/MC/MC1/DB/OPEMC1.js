...
export default function OPEMC1() {
  ...
  const fetchOpenComplaints = async () => {
    try {
      ...
      if (response && response.success && response.data.length > 0) {
        // Transform the PHP data and filter only open complaints
        const transformedData = response.data
          .filter(complaint => complaint.status.toLowerCase() === 'open')
          .map(complaint => ({
            id: complaint.id,
        ...
        console.log('❌ No open complaints found in API');
        setComplaints([]);
  ...
  const getStatusClass = (status) => {
    ...
    if (statusLower === 'in progress' || statusLower === 'inprogress') return 'status-inprogress';
  ...
  return (
    <div>
      ...
      <div className="total-main">
        ...
        <h1 className="total-title">Open Complaints - Greater Chennai Corporation</h1>
        <p className="total-subtitle">Complete list of open complaints with filtering and search options</p>
        ...
        {loading ? (
          <div className="loading">Loading open complaints...</div>
        ) : (
          <>
              ...
              {currentComplaints.map((complaint, index) => (
                <div key={complaint.id || index} className="complaint-card">
                  <div className="complaint-header">
                    <span className="complaint-id">#{complaint.id || `OPEN-${startIndex + index + 1}`}</span>
            ...
            {filteredComplaints.length === 0 && !loading && (
              <div style={{ textAlign: 'center', padding: '40px', color: '#666' }}>
                No open complaints found{searchTerm || categoryFilter !== 'all' ? ' matching your filters' : ''}.
              </div>
            )}
