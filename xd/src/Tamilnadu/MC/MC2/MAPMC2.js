...
const center = [13.0827, 80.2707]; // Example: Chennai (update for MC2 city)

const getRandomLocations = (count) => {
  const locations = [];
  for (let i = 0; i < count; i++) {
    const lat = Math.random() * (13.2 - 12.9) + 12.9;
    const lng = Math.random() * (80.3 - 80.1) + 80.1;
    locations.push({ lat, lng, address: `Random Location ${i + 1}` });
  }
  return locations;
};
...
export default function MAPMC2() {
