...
export default function WaterSupplyDepartment() {
  ...
  useEffect(() => {
    fetchWaterSupplyComplaints();
  }, []);

  const fetchWaterSupplyComplaints = async () => {
    try {
      setLoading(true);
      setError(null);
      
      const response = await getComplaints('MC1');
      console.log('🏢 Water Supply Dept - API Response:', response);
      
      if (response && response.success && response.data) {
        // Filter complaints for Water Supply department
        const waterSupplyComplaints = response.data.filter(complaint => 
          complaint.category && complaint.category.toLowerCase().includes('water') ||
          complaint.department && complaint.department.toLowerCase().includes('water')
        );
        
        console.log('💧 Filtered Water Supply Complaints:', waterSupplyComplaints);
        setComplaints(waterSupplyComplaints);
        setTotalComplaints(waterSupplyComplaints.length);
      } else {
        setError('No data received from API');
      }
    } catch (err) {
      console.error('🚨 Error fetching water supply complaints:', err);
      setError(err.message);
      
      // Fallback mock data for water supply department
      const mockWaterSupplyComplaints = [
        {
          id: 'WAT001',
          description: 'No water supply in Block A for 3 days',
          status: 'Open',
        ...
        {
          id: 'WAT002', 
          description: 'Water pipe leakage on Park Street',
          status: 'In Progress',
          category: 'Water Supply',
          date: '2025-09-21',
          department: 'Water Supply'
      ...
      setComplaints(mockWaterSupplyComplaints);
      setTotalComplaints(mockWaterSupplyComplaints.length);
    } finally {
      setLoading(false);
    }
  };
    ...
    try {
      ...
      if (complaintId) {
        ...
        try {
          ...
          await fetchWaterSupplyComplaints();
          
        } catch (apiError) {
          console.warn(`⚠️ API update failed for ${complaintId}:`, apiError.message);
        }
        
        // No need to update local state - data will be refreshed from database
        ...
        if (openComplaintsToUpdate.length > 0) {
          ...
          for (const complaint of openComplaintsToUpdate) {
            try {
              ...
              // Call API to update status in database
              await updateComplaintStatusAPI(complaint.id, newStatus, priority, 'MC1');
              console.log(`✅ Updated complaint ${complaint.id} in database`);
              successCount++;
          ...
          if (successCount > 0) {
            console.log(`✅ Updated ${successCount} complaints in database`);
          }
    ...
    return (
      <>
        ...
        <main className="main-content">
          <div className="loading">💧 Loading Water Supply Department data...</div>
        </main>
      </>
    );
  }
    ...
    <>
      ...
      <main className="main-content">
        ...
        <div className="stats-container">
          ...
          <div className="stats-label">Total Water Supply Complaints</div>
        ...
        <div className="complaints-section">
          ...
          {complaints.length === 0 ? (
            <div className="no-data">No water supply complaints found</div>
          ) : (
