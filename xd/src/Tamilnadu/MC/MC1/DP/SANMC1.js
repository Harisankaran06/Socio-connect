...
export default function SanitationDepartment() {
  ...
  const fetchSanitationComplaints = async () => {
    try {
      setLoading(true);
      setError(null);
      
      const response = await getComplaints('MC1');
      console.log('🏢 Sanitation Dept - API Response:', response);
      
      if (response && response.success && response.data) {
        // Filter complaints for Sanitation department
        const sanitationComplaints = response.data.filter(complaint => 
          complaint.category && complaint.category.toLowerCase().includes('sanitation') ||
          complaint.category && complaint.category.toLowerCase().includes('cleaning') ||
          complaint.category && complaint.category.toLowerCase().includes('waste') ||
          complaint.department && complaint.department.toLowerCase().includes('sanitation')
        );
        
        console.log('🧹 Filtered Sanitation Complaints:', sanitationComplaints);
        setComplaints(sanitationComplaints);
        setTotalComplaints(sanitationComplaints.length);
      } else {
      ...
      // Fallback mock data for sanitation department
      const mockSanitationComplaints = [
        {
          id: 'SAN001',
        ...
        {
          id: 'SAN002', 
          description: 'Overflowing dustbin near park',
          status: 'In Progress',
          category: 'Sanitation',
          date: '2025-09-21',
          department: 'Sanitation'
        ...
        {
          id: 'SAN003',
          description: 'Street cleaning not done properly',
          status: 'Resolved',
          category: 'Sanitation', 
          date: '2025-09-20',
          department: 'Sanitation'
        ...
        {
          id: 'SAN005',
          description: 'Public toilet not maintained',
          status: 'In Progress',
          category: 'Sanitation',
          date: '2025-09-18',
          department: 'Sanitation'
      ...
      setComplaints(mockSanitationComplaints);
      setTotalComplaints(mockSanitationComplaints.length);
    } finally {
      setLoading(false);
    }
  };
      ...
      if (complaintId) {
        ...
        try {
          ...
          if (updateResult && updateResult.success) {
            console.log(`🎉 Database update confirmed for complaint ${complaintId}:`);
          ...
          console.log(`🔄 Refreshing data from database to show persistent changes...`);
          await fetchSanitationComplaints();
    ...
    return (
      <>
        ...
        <main className="main-content">
          <div className="loading">🧹 Loading Sanitation Department data...</div>
        </main>
      </>
    );
  }
    ...
    <>
      ...
      <main className="main-content">
        ...
        <header className="header">
          <h1>
            <span className="material-icons" style={{fontSize:'2rem'}}>cleaning_services</span>
        ...
        <div className="stats-container">
          ...
          <div className="stats-label">Total Sanitation Complaints</div>
        ...
        <div className="complaints-section">
          ...
          {complaints.length === 0 ? (
            <div className="no-data">No sanitation complaints found</div>
          ) : (
