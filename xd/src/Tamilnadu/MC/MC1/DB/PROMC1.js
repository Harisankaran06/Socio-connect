...
export default function PROMC1() {
  ...
  const fetchInProgressComplaints = async () => {
    try {
      ...
      if (response && response.success && response.data.length > 0) {
        // Transform the PHP data and filter only in progress complaints
        const transformedData = response.data
          .filter(complaint => {
            const status = complaint.status.toLowerCase();
            return status === 'in progress' || status === 'inprogress' || status === 'progress';
          })
          .map(complaint => ({
            id: complaint.id,
        ...
        console.log('❌ No in progress complaints found in API');
        setComplaints([]);
  ...
  return (
    <div>
      ...
      <div className="total-main">
        ...
        <h1 className="total-title">In Progress Complaints - Greater Chennai Corporation</h1>
        <p className="total-subtitle">Complete list of in progress complaints with filtering and search options</p>
        ...
        {loading ? (
          <div className="loading">Loading in progress complaints...</div>
        ) : (
          <>
              ...
              {currentComplaints.map((complaint, index) => (
                <div key={complaint.id || index} className="complaint-card">
                  <div className="complaint-header">
                    <span className="complaint-id">#{complaint.id || `PROG-${startIndex + index + 1}`}</span>
            ...
            {filteredComplaints.length === 0 && !loading && (
              <div style={{ textAlign: 'center', padding: '40px', color: '#666' }}>
                No in progress complaints found{searchTerm || categoryFilter !== 'all' ? ' matching your filters' : ''}.
              </div>
            )}
